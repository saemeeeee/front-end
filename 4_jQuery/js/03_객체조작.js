...
$("#content2").html($('#content1').html()); // 네이버로
//$("#content2").text($('#content1').html()); // <a>네이버로<a>
...
document.querySelector("#area1").appendChild(p); // p태그를 자식으로 추가하겠다

const p2 = $("<p>").html("제이쿼리로 추가");
$("#area1").append(p2);

// 2) 요소 추가 1 -> 부모가 주체
// $(A).append(B) : A 요소의 자식 요소로 B요소를 뒷부분에 추가한다.
...
$("#remove").click(() => {
    ...
    $("#remove-result").append(item);  // 이벤트 호버 사라짐
});
// 2) detach - 
$("#detach").click(() => {
    let item = $("#item2").detach();
    $("#remove-result").append(item); // 이벤트 호버 그대로 있음
});
// 3) empty
$("#empty").click(() => {
    $("#remove-test").empty();
